...
if problem.options.useIPOPT == false
    fprintf('Solving with fmincon...\n')
    if flagGradObj
        P.objective = @(z)( ...
...
%%%% Set up problem for IPOPT instead of fmincon:
elseif problem.options.useIPOPT == true
    fprintf('Solving with IPOPT...\n')
    % Initial Guess
    x0 = zGuess;

    % Get funcs of the objective, gradient, constraints, constraint
    % jacobian, and jacobian sparsity
    P.objective = @(z)( ...
        myObjGrad(z, pack, F.pathObj, F.bndObj, F.weights, gradInfo) );   %Analytic gradients
    [~, objGradInit] = P.objective(zGuess);
    sparsityPattern.objective = (objGradInit~=0)';  % Only used for visualization!  

    funcs.objective = @(z) chooseFirstOutput(z, P.objective);
    funcs.gradient = @(z) chooseSecondOutput(z, P.objective);
    ...
    funcs.constraints = @(z) chooseFirstTwoOutputs(z, P.nonlcon);
    funcs.jacobian = @(z) sparse(chooseLastTwoOutputs(z, P.nonlcon)); 

    % Must designate sparsity structure, changes with number of segments
    % (if doing segs != 100, can get sparsity from a run with same segs on
    % fmincon and placing a debug point here)
    if problem.options.trackXB == true
        load('IPOPT\Sparsity_100segs_YBandXB.mat', 'jacSparsity'); 
        
    else
        load('IPOPT\Sparsity_100segs_YBOnly.mat', 'jacSparsity'); 
    end

    funcs.jacobianstructure = @() jacSparsity;

    % Set bounds and IPOPT solve options
    sizeIneq = size(cstIneqInit, 2);
    sizeEq = size(cstEqInit, 2);
    options.lb = zLow;
    options.ub = zUpp;
    options.cl = [ones(sizeIneq, 1)*-Inf; zeros(sizeEq, 1)];
    options.cu = [zeros(sizeIneq, 1); zeros(sizeEq, 1)];
%     options.zl = [];
%     options.zu = [];
%     options.lambda = [];
...
%     options.ipopt.derivative_test = [];
    options.ipopt.max_iter = Opt.nlpOpt.MaxIter;
    options.ipopt.acceptable_constr_viol_tol = 3;
    options.ipopt.acceptable_iter = Opt.nlpOpt.MaxIter - 1;

    %%% Call IPOPT
    tic;
    [zSoln, output] = ipopt(x0,funcs,options);
    [tSoln,xSoln,uSoln] = unPackDecVar(zSoln,pack);
    nlpTime = toc;
...
%%%%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~%%%%
%%%%            Additional Sub-Functions for IPOPT                     %%%%
%%%%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~%%%%


%%%% ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ %%%%

function [out] = chooseFirstOutput(in, func)
    [out, ~] = func(in);
end

function [out] = chooseSecondOutput(in, func)
    [~, out] = func(in);
end

function [out] = chooseFirstTwoOutputs(in, func)
    [one, two, ~, ~] = func(in);
    out = [one; two];
end

function [out] = chooseLastTwoOutputs(in, func)
    [~, ~, one, two] = func(in);
    out = [one, two]';
end
