...
for i=1:length(opt)
    ...
    switch OPT_method
        ...
        case 'ipopt'
            OPT.ipopt = defaults_ipopt(opt(i).defaultAccuracy);
        otherwise
...
function OPT_ipopt = defaults_ipopt(accuracy)

OPT_ipopt.bounds.phase.integral.lower = -inf;
OPT_ipopt.bounds.phase.integral.upper = inf;
OPT_ipopt.guess.phase.integral = 0;

OPT_ipopt.name = 'OptimTraj_ipopt';
OPT_ipopt.auxdata = [];
OPT_ipopt.nlp.solver = 'ipopt'; % {'ipopt','snopt'}
OPT_ipopt.derivatives.dependencies = 'full';  %�full�, �sparse� or �sparseNaN�
OPT_ipopt.derivatives.supplier = 'sparseCD'; %'sparseCD';  %'adigator'
OPT_ipopt.derivatives.derivativelevel = 'first'; %'second';
OPT_ipopt.mesh.method = 'hp-PattersonRao';
OPT_ipopt.method = 'RPM-Integration';
OPT_ipopt.mesh.phase.colpoints = 10*ones(1,10);
OPT_ipopt.mesh.phase.fraction = ones(1,10)/10;
OPT_ipopt.scales.method = 'none'; % { 'none' , automatic-hybridUpdate' , 'automatic-bounds';

switch accuracy
    case 'low'
        OPT_ipopt.mesh.tolerance = 1e-2;
        OPT_ipopt.mesh.maxiterations = 0;
    case 'medium'
        OPT_ipopt.mesh.tolerance = 1e-3;
        OPT_ipopt.mesh.maxiterations = 1;        
    case 'high'
        OPT_ipopt.mesh.tolerance = 1e-4;
        OPT_ipopt.mesh.maxiterations = 3;
    otherwise
        error('Invalid value for options.defaultAccuracy')
end
